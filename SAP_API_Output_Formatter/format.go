...
func ConvertToGeneral(raw []byte, l *logger.Logger) ([]General, error) {
	...
	for i := 0; i < 10 && i < len(pm.D.Results); i++ {
		...
		general = append(general, General{
			...
			ToContact:                     data.ToContact.Deferred.URI,
		})
	}
...
func ConvertToToContact(raw []byte, l *logger.Logger) ([]ToContact, error) {
	pm := &responses.ToContact{}

	err := json.Unmarshal(raw, pm)
	if err != nil {
		return nil, xerrors.Errorf("cannot convert to ToContact. unmarshal error: %w", err)
	}
	if len(pm.D.Results) == 0 {
		return nil, xerrors.New("Result data is not exist")
	}
	...
	toContact := make([]ToContact, 0, 10)
	for i := 0; i < 10 && i < len(pm.D.Results); i++ {
		data := pm.D.Results[i]
		toContact = append(toContact, ToContact{
			BusinessPartner:        data.BusinessPartner,
