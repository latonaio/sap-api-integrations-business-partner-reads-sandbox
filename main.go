...
import (
	sap_api_caller "sap-api-integrations-business-partner-reads-rmq-kube/SAP_API_Caller"
	sap_api_input_reader "sap-api-integrations-business-partner-reads-rmq-kube/SAP_API_Input_Reader"
	"sap-api-integrations-business-partner-reads-rmq-kube/config"
...
func callProcess(caller *sap_api_caller.SAPAPICaller, msg rabbitmq.RabbitmqMessage) (err error) {
	...
	businessPartner, businessPartnerRole, addressID, bankCountryKey, bankNumber, bPName := extractData(msg.Data())
	accepter := getAccepter(msg.Data())
	caller.AsyncGetBP(businessPartner, businessPartnerRole, addressID, bankCountryKey, bankNumber, bPName, accepter)
	return nil
}

func extractData(data map[string]interface{}) (businessPartner, businessPartnerRole, addressID, bankCountryKey, bankNumber, bPName string) {
